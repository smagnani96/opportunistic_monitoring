...
class Swap(Probe):
    ...
    def __post_init__(self):
        ...
        if self.test_type.value in [TestType.COMPILATION_NORMAL.value, TestType.COMPILATION_SWAP.value]:
            self.ingress.code = self.ingress.code.replace("DECL_PLACEHOLDER", '\n'.join(['BPF_TABLE("array", int, int, A{}, 1){};'.format(i, "__attributes__((SWAP))" if self.test_type.value == TestType.COMPILATION_SWAP.value else "") for i in range(self.nmaps)]))
        else:
            self.ingress.code = self.ingress.code.replace("DECL_PLACEHOLDER", 'BPF_TABLE("array", int, u64, PACKETS, 1){};'.format("__attributes__((SWAP))" if self.test_type.value in [TestType.COMPILATION_SWAP.value, TestType.THROUGHPUT_SWAP.value] else ""))
