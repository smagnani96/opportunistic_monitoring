...
def _parse_arguments():
    ...
    parser.add_argument('interface', help='local interface to receive the remote attacks', type=str)
    parser.add_argument('ssh_login', help='ssh remote login', type=str)
    parser.add_argument('timeout', help='timeout to wait before stopping the test', type=int)
    ...
    for mode in [BPF.XDP, BPF.SCHED_CLS]:
        ...
        for i in range(1, 1+ len(Nprobe.NPROBE_FEATURES)):
            ...
            for _ in range(args["ntimes"]):
                ctr.create_probe(__package__, "probe", interface=args["interface"], mode=mode, flags=XDPFlags.DRV_MODE, nfeatures=i)
