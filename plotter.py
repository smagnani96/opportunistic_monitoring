...
def plot_swap_overhead():
    ...
    ax.set_xticks(ticks)
    ax.set_ylim(top=1.4)
    _do_set_and_store(fig, ax, 'N° maps', 'Time (s)', os.path.join(
        "results", "swap", "chartswapcompilation"))
    ...
    _do_set_and_store(fig1, ax1, 'Swap Frequency (s)', 'Throughput (Mpps)',
                      os.path.join("results", "swap", "chartswapthroughput"))
...
def plot_adaptiveness_comp():
    ...
    ax0.bar([k - 0.25 for k in ticks], ad_cost,
            width=0.25, label="Fully-Adaptive")
    ax0.bar(ticks, [non_ad_cost]*len(ticks), width=0.25,  label="Non-Adaptive")
    ax0.bar([k + 0.25 for k in ticks], [non_ad_cost] *
            len(ticks), width=0.25,  label="Adaptive")
    ax0.set_xticks(ticks)
    ax0.set_xticklabels(
        list(adaptiveness[str(BPF.XDP)][AdaptivenessType.FULLY.value].keys()))
    ax0.set_ylim(top=non_ad_cost + 4)
    _do_set_and_store(fig0, ax0, 'N° Features', 'Memory (MB)', os.path.join(
        "results", "adaptiveness", "chartadaptivenessmemory"))
    ...
    ax.set_xticks(ticks)
    ax.set_ylim(top=43)
    _do_set_and_store(fig, ax, 'N° Features', 'Throughput (Mpps)', os.path.join(
        "results", "adaptiveness", "chartadaptivenessthroughput"))
...
def plot_erase():
    ...
    for mt in MapType:
        ...
        _do_set_and_store(fig, ax, 'N° Entries', 'Time (ms)', os.path.join(
            "results", "erase", "charterase{}".format(mt.value)))
...
def plot_netflow_comp():
    ...
    ax.bar([k - 0.3 for k in ticks], [real_nprobe]
           * len(ticks), label="nProbe", width=0.3)
    for mode, name, ss in [(BPF.SCHED_CLS, "TC", 0), (BPF.XDP, "XDP", 0.3)]:
        vals = [np.array(x)/DURATION for x in nprobe[str(mode)].values()]
        ci = [1.96 * np.std(x)/(len(x)**(1/2)) for x in vals]
        vals = [np.mean(x) for x in vals]
        ax.bar([k + ss for k in ticks], vals, yerr=ci,
               label="Fully-Adaptive {}".format(name), width=0.3)
    ax.set_xticks(ticks)
    _do_set_and_store(fig, ax, 'Feature ID', 'Throughput (Mpps)', os.path.join(
        "results", "nprobe", "chartnprobethroughput"))
    ...
    fig0, ax0 = plt.subplots()
    ax0.bar([k - 0.15 for k in ticks], [netflow_cost]
            * len(ticks), width=0.3,  label="nProbe")
    ax0.bar([k + 0.15 for k in ticks], netflow_ad_cost,
            width=0.3, label="Fully-Adaptive")
    ax0.set_xticks(ticks)
    ax0.set_ylim(top=netflow_cost + 0.5)
    _do_set_and_store(fig0, ax0, 'Feature ID', 'Memory (MB)',
                      os.path.join("results", "nprobe", "chartnprobememory"))
